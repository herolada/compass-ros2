...
protected:
  CompassTransformerNodelet() {
    ...
    this->targetType = this->get_parameter_or<OutputType>("target_type", this->targetType);
    ...
    this->converter = std::make_shared<CompassConverter>(log, this->get_parameter_or<bool>("strict", true));
    ...
    switch (this->targetType)
    {
      case OutputType::Imu:
        ...
        this->pub = this->create_generic_publisher(
          targetAppendSuffix ? outputTopicSuffix :  "azimuth_out", "imu", queue_size);
        break;
      case OutputType::Pose:
        ...
        this->pub = this->create_generic_publisher(
          targetAppendSuffix ? outputTopicSuffix :  "azimuth_out", "pose", queue_size);
        break;
      case OutputType::Quaternion:
        outputTopicSuffix = getAzimuthTopicSuffix<geometry_msgs::msg::QuaternionStamped>(
          targetUnit, targetOrientation, targetReference);
        this->pub = this->create_generic_publisher(
          targetAppendSuffix ? outputTopicSuffix :  "azimuth_out", "quaternion", queue_size);
        break;
      default:
        outputTopicSuffix = getAzimuthTopicSuffix<Az>(targetUnit, targetOrientation, targetReference);
        this->pub = this->create_generic_publisher(
          targetAppendSuffix ? outputTopicSuffix :  "azimuth_out", "azimuth", queue_size);
        break;
    }

    this->azimuthInput = std::make_unique<UniversalAzimuthSubscriber>(log, clock, this, "azimuth_in", queue_size);
    ...
    if (subscribeFix)
    {
      this->fixInput = std::make_unique<message_filters::Subscriber<Fix>>(this, "fix", queue_size);
      this->compassFilter->connectFixInput(*this->fixInput);
    }

    if (subscribeUTMZone)
    {
      this->utmZoneInput = std::make_unique<message_filters::Subscriber<std_msgs::msg::Int32>>(this, "utm_zone", queue_size);
    ...
    else
    {
      this->tfFilter = std::make_unique<tf2_ros::MessageFilter<Az>>(
        *this->compassFilter, this->buffer, targetFrame, queue_size, this);
    ...
    RCLCPP_INFO(log, "Publishing azimuth to topic %s (type %s).",
      this->pub->get_topic_name(), outputTypeToString(this->targetType).c_str());
  }
  ...
  void transformAndPublish(const Az::ConstPtr& msg)
  {
    try
    {
      ...
      *outMsg = this->buffer.transform(*msg, this->targetFrame, rclcpp::Duration(0.1,0));
      this->publish(outMsg);
    }
  ...
  tf2_ros::Buffer buffer {tf2_ros::Buffer(this->get_clock())};
};
